...
const DialogDetailArticle = ({
	...
	console.log("dataa", dataArticle?.name);
	return (
		<DialogModal
			open={open}
			onOpenChange={() => {
			...
			content={
				<div className="mt-5">
					<div className="flex flex-row justify-between gap-1">
						...
						<CellText
							color={dataArticle?.status == "draft" ? "red-500" : "white"}
							className={cn(
								...
								dataArticle?.status === "published" && "bg-[#00A30E]"
							)}
						>
							{dataArticle?.status === "draft" ? "Draft" : "Published"}
						</CellText>
					</div>
					<div className="max-h-[500px] overflow-y-scroll my-3">
						<div className="mt-3">
							<img
								src={dataArticle?.image_path || "-"}
